...
public class SudokuSolver_37 {
    ...
    public static boolean valid(char[][] board, int x, int y) {
    	// 가로 확인 
    	boolean check[] = new boolean[10]; // 1~9 숫자 중복 확인
    	for(int i=0; i<9; i++) {
    		if(board[x][i] == '.') continue;
    		if(check[board[x][i] - '0'] == true) return false;
    	...
    	for(int i=0; i<9; i++) {
    		...
    		if(check[board[i][y] - '0'] == true) return false;
    	...
    	for(int i=x/3 * 3 ; i<x/3 * 3 + 3; i++) {
    		for(int j=y/3 * 3; j<y/3 *3 + 3; j++) {
        		...
        		if(check[board[i][j] - '0'] == true) return false;
